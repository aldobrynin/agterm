...
public enum ControlResolve {
    ...
    /// Derive the control socket path. With `stateDir` (the `AGT_STATE_DIR` value, if set) it is
    /// `<stateDir>/agt.sock`; otherwise `<appSupport>/agt.sock`. The app and the CLI both call this
    ...
    public static func socketPath(stateDir: String?, appSupport: String) -> String {
        ...
        return (base as NSString).appendingPathComponent("agt.sock")
    }
}
